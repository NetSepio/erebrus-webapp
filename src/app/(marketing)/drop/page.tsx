...
// Erebrus Drop — nearby local transfer first, optional IPFS storage second.
const features = [
  { glyph: "⬡", title: "Local Drop Rooms", desc: "Move files, photos, and pasted text between nearby devices over Wi-Fi or hotspot without uploading to cloud storage." },
  ...
  { glyph: "◎", title: "Optional IPFS storage", desc: "When you need a durable link, store files on community-run Erebrus nodes and share by opaque URL or CID." },
  { glyph: "◇", title: "Private storage encryption", desc: "Private stored files are encrypted on your device before upload. Others can't read them." },
];
    ...
    <>
      ...
      <section className="mx-auto grid max-w-[1180px] items-center gap-12 px-4 py-16 md:grid-cols-2 md:px-8 md:py-20">
        <div>
          ...
          <p className="mt-5 max-w-[540px] text-lg leading-relaxed text-[var(--text-2)]">
            Create a Drop Room, scan the QR code from another device, and move files, photos, or pasted text directly over Wi-Fi or hotspot. IPFS storage and public links are there when you need a durable share, not as the default path.
          </p>
          <div className="mt-8 flex flex-wrap gap-3.5">
            <Link href="/storage" className="contents">
              <AccentButton className="!px-6 !py-4">Start Drop Room</AccentButton>
            </Link>
            <Link href="/vpn">
      ...
      <section id="features" className="mx-auto max-w-[1180px] px-4 py-16 md:px-8">
        <div className="mb-12 text-center">
          ...
          <h2 className="text-3xl font-bold md:text-[44px]">Transfer nearby first</h2>
        </div>
        ...
        <Card className="flex flex-col items-center p-10 text-center md:flex-row md:text-left">
          <div className="flex-1">
            ...
            <p className="mt-3 text-[var(--text-2)]">
              Install Drop on your devices, create a local room, scan the QR code, and transfer directly across your Wi-Fi or hotspot. Use IPFS storage only when you want a persistent public or private share.
            </p>
          </div>
          <AppDownloadLinks
            productName="Erebrus Drop"
