...
import { prepStops } from "@/config/gradient-config";
...
export default function GradientBlinds({
  ...
  useEffect(() => {
    ...
    void mainImage( out vec4 fragColor, in vec2 fragCoord )
    {
      ...
      // Concentric circles pattern from top-right corner with larger spacing
      vec2 topRight = vec2(aspect, 1.0);
      float distFromTopRight = length(p - topRight);
      float stripe = fract(distFromTopRight * max(uBlindCount, 1.0) * 0.15);
    ...
    const loop = (t: number) => {
      ...
      if (!paused && programRef.current && sceneRef.current) {
        try {
